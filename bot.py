...
async def build_and_launch(query, context, user_id, chat_id):
    ...
    try:
        await query.edit_message_text("⚡ **Generating Questions via Gemini AI... Please wait.**")
    except Exception:
        pass
    ...
    try:
        ...
        await context.bot.send_message(chat_id, f"❌ Failed to fetch questions: {e}\nType `/quiz` to restart.")
        return
    ...
    await context.bot.send_message(chat_id, "🔥 **QUIZ LOADED SUCCESSFULLY! LET'S GO!** 🔥")
